...
//      }
    ...
    /**
     * 构建url请求地址，子类可复写，返回优先级为model.url是一个完整的url结构 -> Orange.restfullApi|restfullApiHttps +model.url
    ...
    buildurl: function () {
      ...
      if( this.protocol == "http" && Orange.restfullApi){
        restfulApi = Orange.restfullApi;
      }else if(this.protocol == "https" && Orange.restfullApiHttps){
        restfulApi = Orange.restfullApiHttps;
      }
          ...
          for (var i = 0, len = this.validates.length; i < len; i++) {
            ...
            if ((typeof validates === 'boolean')) {
              ...
              if (validates && validates.overdue) {
                require([(Orange.isHybrid || Orange.isInCtripApp) ? 'cHybridMember' : 'cWebMember'], function (Member) {
