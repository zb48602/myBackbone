define(['cHybridGuider'], function(Guider){
  ...
  Orange.localRoute = {
    ...
    addConfig: function (obj) {
      for (var urlschema in obj) {
        if (obj.hasOwnProperty(urlschema)) {
          Orange.localRoute.config[urlschema] = obj[urlschema];
        }
      }
    },
    ...
    mapUrl: function (url) {
      var ret = '', lc = 0;
      _.each(Orange.localRoute.config, function(item, urlSchema){
        if (Orange.localRoute.config.hasOwnProperty(urlSchema)) {
          var parseRet = Orange.schema2re(urlSchema, url);
          if (parseRet.reStr && parseRet.param) {
            if (parseRet.reStr.length > lc) {
              lc = parseRet.reStr.length;
              ret = Orange.localRoute.config[urlSchema];
            }
          }
        }
      })      
  ...
  if (window.OrangeLocalroute) {
    Orange.localRoute.addConfig(window.OrangeLocalroute);
    var el = document.getElementById("OrangeLocalroute");
    if (el) {
      Orange.weinre = el.getAttribute("OrangeWeinre");
      Orange.ip = el.getAttribute("OrangeIP");
      Orange.chanal = el.getAttribute("OrangeChanal");
    }
  }
