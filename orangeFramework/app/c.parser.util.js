...
define(function(){
  ...
  ParseUtil.getPageParams = function(url, urlschema) {
    ...
    _.each(urlschema, function(item){
      ...
      var parseRet = Orange.schema2re(item, url);
  ...
  ParseUtil.parseDepend = function(configStr) {
    var ajaxDataMatch = configStr.match(/Orange.D\(([\'\"])(.*?)([\'\"])\)(.*?)(,|\s)/g), dataexpr = [];
    if (ajaxDataMatch)
    {
  ...
  Orange.getModels = function(pageConfig)
  {
    ...
    _.each(apis, function(api){
      ...
      if((api.runat == Orange.renderAt) || api.runat=="all"){
        ret.push(api);
      }   
      if ('suspend' in api)
      {
      ...
      _.each(dataexpr, function(p)
      {
        ...
        if (JSON.stringify(postdataStr).indexOf(p) > -1 || ParseUtil._containFunc(api.postdata, p) || (api.suspend && api.suspend.indexOf(p) > -1))
        {
          ...
          api.depends.push(eval(p.match(/Orange.D\(([\'\"])(.*?)([\'\"])\)/g)[0].split('Orange.D').join('')));
    ...
    {
      Orange.errorBack = pageConfig.errorBack;
    }
    else
    {
      Orange.errorBack = null;
    }
    return apis;
  } 
   ...
   * @Method Global.Orange.T
   * @example
   * //返回值
   * var domString  = Orange.T("testId")
   * console.log(domString) //" < d i v > < s p a n> 测试字符串</ s p a n></ d i v>"
   */
  Orange.T = Orange._T = function(tmplId,datas)
  {
    if (arguments.length == 1)
    {
      var ret = "";
      var t=Orange.T.lizTmpl[tmplId];
      if (t&&t.runat!=('server')){
        ret = t.text;
      }
      return ret;
    ...
    {
      return _runUnderscore(Orange._T(tmplId),datas);
    }
}
  /**
   * 根据属性Key,获取Url中得参数值
   ...
   * console.log(k)  //5
   */
  Orange.P = function(key, val)
  {
    var ret=null;
    if (_.isUndefined(val)){
      ret= Orange.P.lizParam[key] || Orange.P.lizParam[key.toLowerCase()];
    }else{
      ret=Orange.P.lizParam[key]=val;
    }
    return ret;
  }  
  Orange.schema2re = urlParse;
