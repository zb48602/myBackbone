...
define(['cCoreInherit', 'cLocalStore', 'cCommonStore', 'cParserUtil'], function(cCoreInherit, cStore, CommonStore, ParseUtil){
  ...
  {    
    var configStr = pageDocNode.find('script[type="text/Orange-config"]').text();
    if (!configStr)
    {
  ...
  */
  function getPageTemplates(){    
    var ret={}, templates = pageDocNode.find('script[type="text/Orange-template"]');
  ...
  Orange._initParser = function(url, html)
  {
    pageDocNode = $('<DIV>' + html + '</DIV>'); 
    Orange.T.lizTmpl = getPageTemplates();
    Orange.P.lizParam = getPageParams(url, getPageUrlschema(getPageConfigStr()));
   ...
   * @Method Global.Orange.S
   * @example
   * //参数 storename值
   * 'SALES','SALES_OBJECT','UNION'等等
   */
  Orange.S = function(stroename, key, defaultvalue)
  {
    if (!this.loacaStores)
    {
      this.loacaStores = {};
  ...
  */
  ...
  Orange.render = function(pageConfig, datas, dialogInfo)
  {
    var ret = {
      header: '',
      viewport: ''
    ...
    ret.datas = datas;
    ret.lizTmpl = Orange.T.lizTmpl;
    ret.lizParam = Orange.P.lizParam;
  ...
  Orange.getController = function(pageConfig)
  {
