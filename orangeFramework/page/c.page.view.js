    ...
    var PageView = Backbone.View.extend({
      ...
      */
      forward: function (url, opt) {
        Orange.forward.apply(null, arguments);
      },
      /**
      * 回退至前一页面
      ...
      */
      back: function (url, opt) {
        Orange.back.apply(null, arguments);
      },
      ...
      */
      sendUbt: function (retry) {
        ...
        var url = this.$el.attr('page-url'),
            pageId = Orange.isHybrid ? view.hpageid : view.pageid,
            orderid = Orange.P("orderid") || Orange.P("oid") || "";
        if (pageId === 0) {
          return;
        }
        $('#bf_ubt_orderid').val(orderid);
        var ubtURL = window.location.protocol + '//' + window.location.host + url;
        var refererView = Orange.instance.views[this.referrer];
