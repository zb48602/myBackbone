...
function exec_asyn(service, action, args, success, fail, cancel) {
	...
	if(DEVICE_IS_IOS){
		...
		if(typeof cancel == "function"){
			BestpayHtml5.CALLBACK_CANCEL = cancel;
		}
...
BestPay_Toast.prototype = {
	...
	/**
	 * [makeText 显示toast消息，仅android支持]
	 * @method makeText
	 * @param text {[String]} 文本消息
	 * @param duration {[String]} 消息显示持续时间
	 * @example
	 * 	Toast.makeText("短信民送成功");
	 */
...
BestPay_Contacts.prototype = {
	...
	/**
	 * 获取通讯录联系人列表 android,ios >= 5.0.4
	 * @method getContacts
	 * @return 通讯录联系人列表
	 * @example
	 * 	var contacts = Contacts.getContacts();
	 *	//contacts格式：[{name:"路飞", num:"13737378888"},...]
	 */
	getContacts:function(){
		var result = exec("Contacts","getContacts", {});
		return DEVICE_IS_IOS ? result : JSON.parse(result);
	}
};
...
var BestpayHtml5 = {
	...
	callBackJs : function(result, key) {
		if(DEVICE_IS_IOS){
			...
			BestpayHtml5.RESULT_CODE = status;
			...
			BestpayHtml5.RESULT_CODE = status;
			if (status == BestpayHtml5.CALL_STATUS.SUCCESS) {
				if (typeof this.CALLBACK_SUCCESS == "function")
					setTimeout("BestpayHtml5.CALLBACK_SUCCESS('" + message + "')", 0);
			} else if(status == BestpayHtml5.CALL_STATUS.FAIL) {
				if (typeof this.CALLBACK_FAIL == "function")
					setTimeout("BestpayHtml5.CALLBACK_FAIL('" + message + "')", 0);
			} else if(status == BestpayHtml5.CALL_STATUS.CANCEL) {
				if (typeof this.CALLBACK_FAIL == "function")
					setTimeout("BestpayHtml5.CALLBACK_CANCEL('" + message + "')", 0);
			}
		}
	},
	init: function(){
		BestpayHtml5.topBarClickEvent = BestpayHtml5.getEvent("topBarClick");
		BestpayHtml5.backEvent = BestpayHtml5.getEvent("backClick");		
	},
